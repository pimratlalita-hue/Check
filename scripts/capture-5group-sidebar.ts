...
async function run() {
  ...
  // Navigate to /curriculum (where Academic menu is auto-expanded)
  await page.goto("http://localhost:3010/curriculum", { waitUntil: "networkidle" });
  await page.waitForTimeout(1500);
  await page.screenshot({ path: path.join(SHOT_DIR, "sidebar-04-academic-with-news.png") });

  // Navigate to /news to verify active state inside the submenu
  await page.goto("http://localhost:3010/news", { waitUntil: "networkidle" });
  await page.waitForTimeout(1500);
  await page.screenshot({ path: path.join(SHOT_DIR, "sidebar-05-news-active-in-academic.png") });

  console.log("Captured 5-group sidebar screenshots successfully!");
  await browser.close();
}
